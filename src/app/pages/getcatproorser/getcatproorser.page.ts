...
@Component({
  selector: 'app-getcatproorser',
  templateUrl: './getcatproorser.page.html',
  styleUrls: ['./getcatproorser.page.scss'],
})
export class GetcatproorserPage implements OnInit {
  public fullName:any;
  public userId:any;
  ...
  initialiseTranslation(){
    ...
    this.translate.get('chooseCategory').subscribe((res: string) => {
      this.chooseCategory = res;
    });
  ...
  async functionGetData(typeOp:any,search:any=0){
    this.categoriesService.productsCategories(typeOp,search).then(data=>{
  ...
  async ngOnInit() {
    ...
    await this.functionGetData(this.typeOp)
    await this.notifications();
  }
  async checkUserSearch(event:any){
    await this.functionGetData(this.typeOp,event.target.value)
  }
  selectCat(index:any,catId:any,name:any){
    ...
    this.navCtrl.navigateRoot(['/getsubcatproorser', {catId:this.selectCatId,type:this.typeOp}]);
  }
